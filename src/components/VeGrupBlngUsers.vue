                    ...
                    <template v-if="items">
                        ...
                        >
                            <template v-slot:cell-render="props">
                                <template v-if="props.key==='grupsNames'">
                                    <span v-for="(item, idx) in props.row.grups" :key="idx" style="display:inline-block; line-height:20px; height:20px;">
                                ...
                                <select v-else-if="props.key==='mode'" @change="showVeGrupBlngUsersToggleItemModeByName(props.row.name, $event.target.value)" :disabled="!isEditable">
