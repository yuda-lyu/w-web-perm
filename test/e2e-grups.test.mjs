...
//對齊 users；異於 targets 之 isError 引用未定義變數恆空而走後端 ckKey）；另有 belongUsers / cpemis 兩個關聯欄按鈕；
...
const SEL_TOOLBAR = '[data-fmid="grups-toolbar"]'                              //功能區工具列

function picPath(lang, name) { return `${PICS_DIR}/grups-${lang}-${name}.png` }

//設定語系（test setup 層，非 act-under-test；對齊雙語覆蓋維度）。沿用 e2e-users / e2e-targets 之對稱 buffer 慣例：
...
async function gotoGrups(page) {
    ...
    await clickNavItem(page, grupsLabel) //限定導覽面板內：欄序改版後「管控使用權限」表頭之 eng 文字與「管理權限」選單同字
    await waitUntilExist(page, '群組 ag-grid 列', () => document.querySelectorAll('.ag-row').length > 0, { timeout: 20000 })
    await page.waitForTimeout(500)
}

//—— 互動原語 helpers（沿用 e2e-users / e2e-targets，僅欄位 col-id 改為 grups 的 name/description/cpemis/belongUsers）——
//icon 按鈕（WButtonCircle 無 title/aria，以 mdi icon path 定位）。
//grups 之 plus/copy/trash/upload mdi path 與 users / targets 完全相同（mdiPlus / mdiContentCopy /
...
//系統以持久 CheckYes modal 呈現「操作主要結果」：成功 / 失敗 / errInNames（前端擋存）/ 空（grupAddEmpty）皆 showCheckYes，
...
//對 fail 類只斷言前綴鍵（grupSaveGrupsFail），不含動態 errTemp 字尾。
...
const CASES = [
    {
        //E2E-001：進入群組頁顯示初始清單（預設編輯模式 ON，比照 pilot 之 list-view 截預設態）
        name: 'E2E-001-list-view',
        run: async (page) => {
            await gotoGrups(page)
            return await captureStableWithBox(page, SEL_GRID) //觀看區：群組清單 grid
        },
        semantic: async (page) => {
            const txt = await page.evaluate(() => document.body.innerText)
            assert.ok(txt.includes('權限群組M1'), '應顯示 base seed grup 名稱 權限群組M1')
            assert.ok(txt.includes('權限群組M2') && txt.includes('權限群組M4'), '應顯示多筆 base seed grups')
            ...
            const ks = heads.filter((k) => ["name","description","cpemis","belongUsers"].includes(k))
            assert.deepEqual(ks, ["name","description","cpemis","belongUsers"],
                `主表可見欄序應為 ${["name","description","cpemis","belongUsers"].join(' / ')}（實得 ${JSON.stringify(ks)}）`)
            //對應 spec 語意（A3 欄名）：該欄中文表頭為「管控所屬使用者」
            const hdr = await page.evaluate((k) => {
                const c = document.querySelector(`.ag-header-cell[col-id="${k}"]`)
                return c ? (c.textContent || '').trim() : null
            }, 'belongUsers')
            const want = await page.evaluate((k) => window.$vo.$t(k), 'belongUsers')
            assert.ok(hdr && hdr.includes(want), `${'belongUsers'} 欄表頭應為語系鍵之文字「${want}」（實得「${hdr}」）`)
        },
    },
    {
    ...
    },
    {
        ...
        run: async (page) => {
            ...
            await typeIntoCell(page, 0, 'name', 'E2eGrupE003')
        ...
        semantic: async (page) => {
            await assertModalMsg(page, 'grupSaveGrupsSuccess') //結果 modal 顯示儲存成功
            const g = await page.evaluate(() => (window.$vo.$store.state.grups || []).find((x) => x.name === 'E2eGrupE003'))
            assert.ok(g, '新群組應寫入 DB（store 同步）')
            //userId / timeCreate 已由後端補為實值（非 {待自動給予} 佔位符）
            assert.ok(g && g.userId && !String(g.userId).startsWith('{'), 'userId 應由後端補實值')
            assert.ok(g && g.timeCreate && !String(g.timeCreate).startsWith('{'), 'timeCreate 應由後端補實值')
            const n = await page.evaluate(() => (window.$vo.$store.state.grups || []).length)
    ...
    },
    {
        ...
        run: async (page) => {
            ...
            await saveAndWaitModal(page)
            const s3 = await captureStableWithBox(page, SEL_MODAL) //階段3：儲存成功結果 modal（內含 waitDrawerReady WDrawer 穩定態防護）
            return [
        ...
        semantic: async (page) => {
            ...
            const grups = await page.evaluate(() => (window.$vo.$store.state.grups || []).map((x) => ({ id: x.id, name: x.name })))
            const dup = grups.filter((g) => g.name !== srcName && g.name.includes(srcName) && g.id !== srcId)
            assert.ok(dup.length >= 1, '應有一列為來源 name 帶後綴、id 獨立的複製列')
        },
    },
    {
        //E2E-005：勾選某列 → 刪除 → 儲存 → DB 該列消失（next case 的 resetDb 還原）
        //比照 targets pilot E2E-005：直接刪 base seed row 0，靠每 case 前 resetDb 還原（最簡穩）。
        ...
        semantic: async (page) => {
            ...
            const has = await page.evaluate((nm) => (window.$vo.$store.state.grups || []).some((x) => x.name === nm), delName)
            assert.ok(!has, `base seed 第一列（${delName}）應已刪除`)
            const n = await page.evaluate(() => (window.$vo.$store.state.grups || []).length)
            assert.equal(n, BASE_SEED.length - 1, `DB 應為 base-1（${BASE_SEED.length - 1}）筆`)
            //【端到端不變式：刪 group → 受影響 user 權限樹】base seed row 0 = 權限群組M1，peter 屬之（唯一群組）；
            //刪 M1 後 peter 失去唯一群組 → 解析後權限樹應為空 []（getUserRules 對 peter.cgrups 殘留的 M1 引用找不到群組而忽略）。
            //驗 getPermUserInfor 回傳的 resolved 權限樹，守護「刪群組 → 受影響使用者權限正確縮減」。
            const tree = await getResolvedActiveTargets(page, 'id-for-peter')
            assert.deepEqual(tree, [], `刪除 peter 唯一群組 ${delName} 後，其解析後權限樹應為空（實得 ${JSON.stringify(tree)}）`)
        },
    },
    {
        //E2E-006：新增列後清空 name → name 欄警告 icon（tooltip grupNameEmpty）；
        ...
        run: async (page) => {
            ...
            await typeIntoCell(page, 0, 'name', '') //清空 name（清掉自動帶入的「新權限群組」名）
    ...
    {
        //E2E-007：新增列 name 取既有 grup name（重複）→ name 欄警告 icon（tooltip grupNameDuplicate）；
        //Save 時前端 isError 彙整 errInNames 擋下、不送後端、DB 不變
        ...
        run: async (page) => {
            ...
            await clickAdd(page)
            const s1 = await captureStableWithBox(page, rowBoxSel(0)) //階段1：新增列、自動帶入名（typeIntoCell 前）
            await typeIntoCell(page, 0, 'name', '權限群組M1') //取 base seed 既有 grup name（重複）
    ...
    {
        //E2E-008：點某列 belongUsers 欄按鈕 → 開啟群組成員編輯對話框（VeGrupBlngUsers）。僅驗本流程可觀察事實：對話框出現。
        name: 'E2E-008-belongusers-dialog',
        run: async (page) => {
            await gotoGrups(page)
            const s1 = await captureStableWithBox(page, rowBoxSel(0)) //階段1：來源列（點按鈕前）
            //點第 1 列 belongUsers 欄按鈕（結構 selector：ag-grid col-id）
            await page.locator('.ag-row[row-index="0"] .ag-cell[col-id="belongUsers"] button').first().click()
            //等 VeGrupBlngUsers 對話框（以其標題 grupBlngEditUsers 偵測；isEditable 預設 ON → 編輯態標題）
            await waitUntilExist(page, 'VeGrupBlngUsers 對話框標題', () => {
                const vo = window.$vo
                const label = vo.$t('grupBlngEditUsers')
                return (document.body.innerText || '').includes(label)
            }, { timeout: 15000 })
            await page.waitForTimeout(800)
            const s2 = await captureStableWithBox(page, SEL_MODAL) //階段2：VeGrupBlngUsers 群組成員編輯對話框開啟
            return [
        ...
        semantic: async (page) => {
            ...
            const txt = await page.evaluate(() => document.body.innerText)
            assert.ok(txt.includes(label), `群組成員編輯對話框標題應顯示（${label}）`)
        },
    },
    {
        //E2E-009：點某列 cpemis 欄按鈕 → 開啟群組權限編輯對話框（VeCpemis）。僅驗本流程可觀察事實：對話框出現。
        name: 'E2E-009-cpemis-dialog',
        run: async (page) => {
            await gotoGrups(page)
            const s1 = await captureStableWithBox(page, rowBoxSel(0)) //階段1：來源列（點按鈕前）
            //點第 1 列 cpemis 欄按鈕（結構 selector：ag-grid col-id）
            await page.locator('.ag-row[row-index="0"] .ag-cell[col-id="cpemis"] button').first().click()
            //等 VeCpemis 對話框（以其標題 grupEditCpemis 偵測；isEditable 預設 ON → 編輯態標題）
            await waitUntilExist(page, 'VeCpemis 對話框標題', () => {
                const vo = window.$vo
                const label = vo.$t('grupEditCpemis')
                return (document.body.innerText || '').includes(label)
            }, { timeout: 15000 })
            await page.waitForTimeout(800)
            const s2 = await captureStableWithBox(page, SEL_MODAL) //階段2：VeCpemis 群組權限編輯對話框開啟
            return [
        ...
        semantic: async (page) => {
            ...
            const txt = await page.evaluate(() => document.body.innerText)
            assert.ok(txt.includes(label), `群組權限編輯對話框標題應顯示（${label}）`)
        },
    },
    {
        ...
        run: async (page) => {
            await gotoGrups(page)
            await clickAdd(page)
            await typeIntoCell(page, 0, 'name', 'E2eGrupE010')
        ...
        semantic: async (page) => {
            await assertModalMsg(page, 'grupSaveGrupsFail') //結果 modal 顯示儲存失敗（前綴）
            const has = await page.evaluate(() => (window.$vo.$store.state.grups || []).some((x) => x.name === 'E2eGrupE010'))
            assert.ok(!has, 'token 失效，群組不應寫入 DB')
            const n = await page.evaluate(() => (window.$vo.$store.state.grups || []).length)
...
async function generateBaseline() {
    console.log('=== 產製 grups baseline 開始 ===')
    ...
    cleanup()
    console.log('=== 產製 grups baseline 完成 ===')
}
...
if (isBaseline) {
    ...
    for (const lang of LANGS) {
        describe(`e2e-grups (${lang})`, function() {
            this.timeout(180000)
            let browser = null
